...
export default function IndividualJobView({ perspective, id, source }: Props) {
  ...
  const renderJob = () => {
    if (job) {
      ...
      const localJobTimestamp = utcToZonedTime(new Date(job.timestamp), userTimeZone);
      const formattedTimestamp = format(new Date(localJobTimestamp), "eeee do MMM yyyy @ p", { timeZone: "Australia/Brisbane" });

      const minutesDifference = differenceInMinutes(currentTimestamp, localJobTimestamp);
      const hoursDifference = differenceInHours(currentTimestamp, localJobTimestamp);
      const daysDifference = differenceInDays(currentTimestamp, localJobTimestamp);
      ...
      if (daysDifference > 0) {
        ...
        timeAgoString = `${minutesDifference} minute${minutesDifference === 1 ? "" : "s"} ago`;
      } else {
      ...
      return (
        <div>
          <j-box bg="ui-200" p="500" radius="md">
            ...
            <j-text variant="label"><strong>Job ID: </strong>{id}</j-text>
            <j-text variant="label"><strong>Date created: </strong>{formattedTimestamp} ({timeAgoString}).</j-text>
            <j-flex j="end" a="center" wrap gap="500" direction="row">
              {/* <j-box p="500" bg="ui-300">Item 1</j-box> */}
              {/* <j-box p="500" bg="ui-300">Item 2</j-box> */}
              <j-box p="500" bg="ui-300"><strong>Status: </strong>{job.jobStatus}</j-box>
            </j-flex>
            <br></br>
